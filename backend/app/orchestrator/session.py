...
asyncio.gather fan-out runs all ResearcherAgents in parallel. The CogneeClient
write lock (asyncio.Lock) serializes the actual remember() calls inside each
researcher to prevent Kuzu embedded-graph commit conflicts.

return_exceptions=True ensures a failing researcher never aborts siblings.
"""
...
class SessionOrchestrator:
    ...
    async def run_session(self, domain: Domain, query: str) -> Answer:
        session_id = uuid.uuid4().hex
        ds = domain.dataset_name

        # Register event queue BEFORE background task starts
        ...
        try:
            ...
            planner = PlannerAgent(
                cognee=self._cognee, llm=llm, emit=emit, session_id=session_id
            )
            ...
            results = await asyncio.gather(
                *[make_researcher().run(st, ds) for st in subtasks],
                return_exceptions=True,  # failed researcher ≠ aborted session
            )

            # Flatten findings, drop exceptions
            findings = [
                f
                for r in results
            ...
            # Log any researcher failures (don't swallow silently)
            for r in results:
    ...
    async def _set_status(
        self, session_id: str, status: str, output: str | None = None
    ) -> None:
        async with async_session_factory() as db:
